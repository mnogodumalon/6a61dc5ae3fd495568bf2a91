...
export default function DebitorPage() {
  ...
  const [records, setRecords] = useState<Debitor[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Debitor | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Debitor | null>(null);
  ...
  async function handleCreate(fields: Debitor['fields']) {
  ...
  return (
    <PageShell
      title="Debitor"
      subtitle={`${records.length} Debitor im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/debitor/${record.record_id}`); }}>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={13} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Debitor. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
